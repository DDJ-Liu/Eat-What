...
{
    /// <summary>
    /// Once per Editor session, sweeps registered configurators and re-runs CheckStatus(attemptAutoRewrite: false)
    /// for any installed client that already has a config on disk. Catches the case where the user updated the
    /// MCP for Unity package while the Editor was closed. Client status is read without modifying external configuration.
    /// MIG63 local policy: Configure is an explicit user operation.
    /// </summary>
    [InitializeOnLoad]
        ...
        private static void RunOnce()
        {
            ...
            foreach (var c in McpClientRegistry.All)
            {
                try
                {
                    ...
                    var before = c.Status;
                    var after = c.CheckStatus(attemptAutoRewrite: false);
            ...
            if (rewrote > 0)
                McpLog.Info($"[StartupConfigRewrite] checked {rewrote} client configuration status(es), without rewriting.");
        }
